...
class WebDevServer {
  ...
  static Future<WebDevServer> start(
    ...
    if (options.configuration.enableInjectedClient) {
      ...
      // Read the entrypoint metadata generated by build_web_compilers if
      // available. This avoids having to hard code 'main.dart' and allows us
      // to initialize the frontend server if the build is fully cached.
      String? canonicalUri;
      if (options.configuration.webHotReload &&
          !options.configuration.release) {
        final entrypointJsonStr = await assetReader.dartSourceContents(
          '.web.entrypoint.json',
        );
        if (entrypointJsonStr != null) {
          try {
            final json = jsonDecode(entrypointJsonStr) as Map<String, dynamic>;
            canonicalUri = json['canonicalUri'] as String?;
          } catch (e) {
            _logger.warning('Failed to parse .web.entrypoint.json', e);
          }
        }
      }
      ...
      final buildSettings = BuildSettings(
        appEntrypoint: Uri.parse(
          canonicalUri ?? '$multiRootScheme:///${options.target}/main.dart',
        ),
